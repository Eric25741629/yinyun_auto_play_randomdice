...
class play(game_view.gameview):
    def __init__(self, d: u2.Device, reader: easyocr.Reader, q: Queue, player='att'):
...
def dicer_sup(d: u2.Device, q: Queue, devices_ip: str):
    ...
    supctrl = ctrl_game(d, reader, q, act='sup', devices_ip=devices_ip)
    supctrl.switch_what_to_do()
    start = q.get()
    if (start == 'start'):
        start = time.time()
        sup_game_ctrl = play(d, reader, q, player='att')
